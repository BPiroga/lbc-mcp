...
async def main() -> int:
    ...
    # Le sous-processus ne recoit qu'un environnement minimal : lui passer nos reglages.
    settings = {k: v for k, v in os.environ.items() if k.startswith("LBC_")}
    server = StdioServerParameters(command=sys.executable, args=["-m", "lbc_mcp"], env=settings)
    async with Client(server) as client:
        try:
            await exercise(client)
        except ToolFailed as exc:
            # Capturee ici : hors du client, elle ressortirait noyee dans un ExceptionGroup.
            print(f"\nECHEC : {exc}")
            return 1
    return 0
...
async def exercise(client: Client) -> None:
    ...
    print("prix croissants :", prices, "->", priced == sorted(priced))
    for ad in search["ads"][:3]:
        print(f"  {ad['price'] or '-':>6} €  {ad['title'][:50]:50}  {ad['city']}  {ad.get('attributes')}")
